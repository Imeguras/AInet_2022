					...
					<form method="POST" action="{{ route('alterprofilesubmit') }}">	
						...
						<div class="row mb-3">
							...
							<!--If user is client-->
								...
								{{--Password --}}
								<label id="password-lbl1" for="password-input1" class="col-md-4 col-form-label text-md-end">
									{{ __('Password') }}
								</label>
								<div class="col-md-6">
									<input id="password-input1" type="password" class="form-control @error('password') is-invalid @enderror" name="password" required autocomplete="new-password">
									@error('password')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
